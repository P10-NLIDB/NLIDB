...
nlp_tokenize = stanza.Pipeline('en', processors='tokenize,mwt,pos,lemma,depparse', tokenize_pretokenized=False,
                               use_gpu=True, download_method=DownloadMethod.REUSE_RESOURCES)  # , use_gpu=False)
...
def normalize_and_build_schema_relations(db: dict):
    ...
    # from column id to table id
    column2table = list(map(lambda x: x[0], db['column_names']))
    # from table id to column ids list
    ...
    tab_mat = np.array(
        [['table-table-generic'] * t_num for _ in range(t_num)], dtype=dtype)
    table_fks = set(map(lambda pair: (
        column2table[pair[0]], column2table[pair[1]]), db['foreign_keys']))
    if not db['table_names'] or not db['column_names']:
        db['relations'] = []
        return db

    for (tab1, tab2) in table_fks:
        if (tab2, tab1) in table_fks:
            tab_mat[tab1, tab2], tab_mat[tab2,
                                         tab1] = 'table-table-fkb', 'table-table-fkb'
        else:
            tab_mat[tab1, tab2], tab_mat[tab2,
                                         tab1] = 'table-table-fk', 'table-table-fkr'
    tab_mat[list(range(t_num)), list(range(t_num))] = 'table-table-identity'

    # relations in columns, c_num * c_num
    col_mat = np.array([['column-column-generic'] *
                       c_num for _ in range(c_num)], dtype=dtype)
    ...
    if len(db['foreign_keys']) > 0:
        ...
        col_mat[col1, col2], col_mat[col2,
                                     col1] = 'column-column-fk', 'column-column-fkr'
    ...
    tab_col_mat = np.array(
        [['table-column-generic'] * c_num for _ in range(t_num)], dtype=dtype)
    col_tab_mat = np.array(
        [['column-table-generic'] * t_num for _ in range(c_num)], dtype=dtype)
    cols, tabs = list(
        zip(*list(map(lambda x: (x, column2table[x]), range(1, c_num)))))  # ignore *
    col_tab_mat[cols, tabs], tab_col_mat[tabs,
                                         cols] = 'column-table-has', 'table-column-has'
    if len(db['primary_keys']) > 0:
        cols, tabs = list(
            zip(*list(map(lambda x: (x, column2table[x]), db['primary_keys']))))
        col_tab_mat[cols, tabs], tab_col_mat[tabs,
                                             cols] = 'column-table-pk', 'table-column-pk'
...
def preprocess_natural_language_question(entry: dict, dataset_name: str, data_idx: int):
    ...
    question = " ".join(quote_normalization(
        dataset_name, data_idx, entry["question_toks"]))
    ...
    entry["final_preprocessed_text_list"].append([0, [w.text for s in doc.sentences for w in s.words], len([
    ...
    if q_num <= MAX_RELATIVE_DIST + 1:
        ...
        starting = q_num - 1
    q_mat = np.array([dist_vec[starting - i: starting - i + q_num]
                     for i in range(q_num)], dtype=dtype)
    entry[f'relations'] = q_mat.tolist()

    tree_mat = np.array(
        [["None-Syntax"] * q_num for _ in range(q_num)], dtype=dtype)
...
def schema_linking(entry: dict, db: dict):
    ...
    q_num, t_num, c_num, dtype = len(question_toks), len(
        table_toks), len(column_toks), '<U100'

    # relations between questions and tables, q_num*t_num and t_num*q_num
    table_matched_pairs = {'partial': [], 'exact': []}
    tab_q_mat = np.zeros((t_num, q_num), dtype=dtype)
    tab_q_mat[:] = 'table-question-nomatch'

    q_tab_mat = np.zeros((q_num, t_num), dtype=dtype)
    q_tab_mat[:] = 'question-table-nomatch'
    max_len = max([len(t) for t in table_toks], default=0)
    index_pairs = list(
        filter(lambda x: x[1] - x[0] <= max_len, combinations(range(q_num + 1), 2)))
    ...
    q_col_mat = np.array([['question-column-nomatch'] *
                         c_num for _ in range(q_num)], dtype=dtype)
    col_q_mat = np.array([['column-question-nomatch'] *
                         q_num for _ in range(c_num)], dtype=dtype)
    ...
    if db_content:
        ...
        for i, (tab_id, col_name) in enumerate(db['column_names_original']):
            ...
            try:
                ...
                cell_values = [[str(float(each))] if is_number(
                    each) else each.lower().split() for each in cell_values]
            except Exception as e:
                print(
                    f"[DB VALUE ERROR] {e} -- db: {db['db_id']}, table: {tab_name}, column: {col_name}")
                continue
...
def process_dataset_entries(dataset, tables, dataset_name, mode, output_path_base=None, used_coref=False):
    ...
    if used_coref and not os.path.exists(os.path.join(output_path_base, f"{mode}_coref.json")):
        wfile = open(os.path.join(output_path_base,
                     f"{mode}_text_list.txt"), "w")
    for idx, entry in tqdm(enumerate(dataset)):
        # if idx > 100:
        #     continue
        if dataset_name in ["spider", "ambiQT", "AMBROSIA"]:
            entry = run_preprocessing_pipeline_on_entry(
                entry, tables[entry['db_id']], dataset_name, idx)
        elif dataset_name in ["cosql", "sparc"]:
            entry = run_preprocessing_pipeline_on_entry(
                entry, tables[entry['database_id']], dataset_name, idx)
        else:
            raise NotImplementedError
        if used_coref and not os.path.exists(os.path.join(output_path_base, f"{mode}_coref.json")):
            wfile.write(str(entry['final_preprocessed_text_list'])+"\n")
        processed_dataset.append(entry)
    safe_pickle_save(processed_dataset, os.path.join(
        output_path_base, f"{mode}.pkl"))
...
def get_dataset_file_paths(data_base_dir, dataset_name, mode):
    ...
    db_dir = os.path.join(data_base_dir, "original_dataset",
                          dataset_name, "database")
    table_data_path = os.path.join(
        data_base_dir, "original_dataset", dataset_name, "tables.json")
    table_out_path = os.path.join(
        data_base_dir, "preprocessed_dataset", dataset_name, "tables.pkl")
    if mode == "train":
        if dataset_name == "spider":
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", dataset_name, "train_spider.json")
        elif dataset_name == "cosql":
            db_dir = os.path.join(
                data_base_dir, "original_dataset", "cosql_dataset", "database")
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", "cosql_dataset/sql_state_tracking/", "cosql_train.json")
            table_data_path = os.path.join(
                data_base_dir, "original_dataset", "cosql_dataset", "tables.json")

        elif dataset_name == "sparc":
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", dataset_name, "train.json")
        elif dataset_name == "ambiQT":
            # TODO: change to train later
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", dataset_name, "ambiqt_data.json")
        elif dataset_name == "AMBROSIA":
            # TODO: change to train later
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", dataset_name, "AMBROSIA_data.json")
        ...
        if dataset_name in ["spider", "sparc"]:
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", dataset_name, "dev.json")
        elif dataset_name == "cosql":
            db_dir = os.path.join(
                data_base_dir, "original_dataset", "cosql_dataset", "database")
            dataset_path = os.path.join(
                data_base_dir, "original_dataset", "cosql_dataset/sql_state_tracking/", "cosql_dev.json")
            table_data_path = os.path.join(
                data_base_dir, "original_dataset", "cosql_dataset", "tables.json")
        ...
        raise NotImplementedError
    dataset_output_path_base = os.path.join(
        data_base_dir, "preprocessed_dataset", dataset_name)
    if not os.path.exists(os.path.join(data_base_dir, "preprocessed_dataset", dataset_name)):
        os.makedirs(os.path.join(data_base_dir,
                    "preprocessed_dataset", dataset_name))
...
def generate_preprocessed_relational_data(data_base_dir, dataset_name, mode, used_coref=False, use_dependency=False, overwrite=False):
    ...
    db_dir, table_data_path, table_out_path, dataset_path, dataset_output_path_base = get_dataset_file_paths(
    ...
    if os.path.exists(table_out_path) and not overwrite:
        ...
        print('Databases preprocessing costs %.4fs.' %
    ...
    if os.path.exists(os.path.join(dataset_output_path_base, f"{mode}.pkl")) and not overwrite:
        ...
        dataset = safe_pickle_load(os.path.join(
            dataset_output_path_base, f"{mode}.pkl"))
    else:
        print("Preprocessing dataset entries...")
        raw_dataset = safe_json_load(dataset_path)
        start_time = time.time()
        dataset = process_dataset_entries(
            raw_dataset, tables, dataset_name, mode, dataset_output_path_base, used_coref)
        print('Dataset preprocessing costs %.4fs.' %
              (time.time() - start_time))
